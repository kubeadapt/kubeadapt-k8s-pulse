...
func UprobeToken(args ProbeArgs) string {
	po := fmt.Sprintf("%s:%***REMOVED***x", args.Path, args.Offset)

	if args.RefCtrOffset != 0 {
		// This is not documented in Documentation/trace/uprobetracer.txt.
		// elixir.bootlin.com/linux/v5.15-rc7/source/kernel/trace/trace.c***REMOVED***L5564
		po += fmt.Sprintf("(%***REMOVED***x)", args.RefCtrOffset)
	}
