...
func (r *Requirement) Matches(ls Labels) bool {
	...
	case selection.GreaterThan, selection.LessThan:
		...
		if len(r.strValues) != 1 {
			klog.V(10).Infof("Invalid values count %+v of requirement %***REMOVED***v, for 'Gt', 'Lt' operators, exactly one value is required", len(r.strValues), r)
			return false
		}
		...
		for i := range r.strValues {
			...
			if err != nil {
				klog.V(10).Infof("ParseInt failed for value %+v in requirement %***REMOVED***v, for 'Gt', 'Lt' operators, the value must be an integer", r.strValues[i], r)
				return false
			}
		}
